# Story Repo 格式规格（v7 草案 0.5 · 0.4 冻结条文 + 0.5 变更）

> 状态：0.5（2026-09-02 作者批准，见文首「0.5 变更」节；其下 0.4 条文除 0.5 节所列偏差外维持冻结）。相对 0.3 的变更：伞目录「定稿/」改名「定稿/」（作者拍板，贴作者心智：草稿在工作区，验收后入定稿）。相对 0.2 的变更：废除大一统 YAML 数据文件，承诺与信息差改为"每条一个 Markdown + 平铺 front matter"（§4.3、§5）；新增防呆方言（§2.2）、修复卡（§10 第 0 态）、"对话即编辑器"不变量（§1.9）。决策记录见 §14。
> 来源：2026-06-10「最正确架构」讨论 + 绞杀式收敛计划 Phase 0。
> 地位：本规格是 v7 的法律文本。代码是格式的派生物——任何实现与本规格冲突时，改实现或改规格，不允许"代码里悄悄多存一份"。

## 0.5 变更（2026-09-02，作者已批准生效）

S19/S23 两章实测与[全面审阅](../reports/2026-09-02-项目全面审阅报告.md)确认的实现形态与 0.4 条文的偏差，按本规格地位条款「改实现或改规格」经作者批准登记如下，与本节所列条文同效力：

1. **§4.5 名册双落点正式化**：迁移器写 `名册.md` 单表（迁移兼容读），settle 写 `名册/<正名>.md` 目录（front matter 正名/别名/首现章）；`v7_cache` 兼读两处、同名目录优先（S24 已实现）。纯 v7 原生新书只有目录形态。
2. **§8 commit 机器协议**：settle 现行 `settle: 第NNNN章 标题`；`ch(NNN):`+条目结转行协议待承诺系统（第 6 条）落地时一并收敛，v7.0 保持现行格式。
3. **§4.1 章 front matter 键**：现行 章号/标题/卷/视角/书内时间/字数/承诺豁免/推进承诺/合同；「开启承诺/兑付承诺」键随承诺系统（v7.1）引入。
4. **§7 决策卡**：实测形态取代固定四段——`title/pov/time_anchor/目标字数/必须覆盖节点/禁区/承诺结转豁免/合同断言/关键实体`（工作区 gitignored 不变）。
5. **§2.2 book.yaml 嵌套例外**：`context_budget:`（`total` + `sections` 两级，S23 配额按书覆盖）是平铺原则的唯一登记例外。
6. **§5 承诺系统 + 缓存 promises/secrets/fingerprints 表**：显式登记为 v7.0 范围缺口，排期 v7.1；机检在落地前仅校验「承诺非空或豁免」。
